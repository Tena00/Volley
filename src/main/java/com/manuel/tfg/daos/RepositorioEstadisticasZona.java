...
public interface RepositorioEstadisticasZona extends JpaRepository<EstadisticasZona,Integer> {
    ...
    @Query("SELECT e FROM EstadisticasZona e WHERE e.partido.idPartido = ?1 AND e.idZona.idZonaCampo = ?2 AND e.jugador.idJugador = ?3")
    EstadisticasZona findByIdPartidoIdZona(Integer idPartido, Integer idZona, Integer idJugador);
